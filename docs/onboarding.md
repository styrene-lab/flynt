# Codex Onboarding, Installation & Migration
...
|----------|----------|---------|--------------|
| macOS | `Codex-{VER}.dmg` | Developer ID (UZBY9DM42N) | Direct download |
| iOS | `Codex-{VER}.ipa` | Apple Development (Q4FM48AWU9) | Ad-hoc / TestFlight |
...
1. Download or build `Codex-{VER}.dmg`
2. Open DMG, drag `Codex.app` to `/Applications`
3. First launch: Gatekeeper may block — right-click > Open, or `xattr -cr /Applications/Codex.app`
...
### Mobile limitations
- **No onboarding UI** — vault must exist at `Documents/Codex` before launch
...
2. Codex creates `.codex/` directory inside the folder
3. SQLite index built from all `.md` files
4. No sync configured — local only until Settings > Sync

**Obsidian compatibility:**
- Wikilinks (`[[note]]`) supported
- TOML frontmatter (`+++...+++`) is Codex-native; YAML (`---...---`) parsed but not written
- `.obsidian/` directory is ignored
- Obsidian plugins are not supported — Codex has its own equivalents
...
3. Codex clones to `~/Documents/{repo-name}/`
...
- Stages all changes > commits as "Codex <codex@local>" > pulls (fast-forward or merge) > pushes
...
No tokens or passwords are stored by Codex.
...
| Dataview queries | Codex query blocks (`TABLE`, `LIST`, `TASK`) |
| Community plugins | Not supported |
| YAML frontmatter | Read but not written (Codex uses TOML `+++`) |
| Vim mode | Not yet |
| PDF/image embeds | Images embedded; PDF not yet |

### Migration steps
1. Copy or point Codex at your Obsidian vault folder
2. Codex indexes everything — `.obsidian/` is ignored
...
3. Mobile app reads the vault from `Documents/Codex`
...
8. **Commit author is "Codex <codex@local>"** — not yet linked to user identity (StyreneIdentity planned)
...
```bash
# 1. Install
open Codex-0.1.0.dmg  # drag to Applications
xattr -cr /Applications/Codex.app  # bypass Gatekeeper if needed
