...
5. Push vault data: `xcrun devicectl device copy to --device <name> --domain-type appDataContainer --domain-identifier io.styrene.flynt --source <vault-dir> --destination Documents/Flynt`
...
### Mobile limitations
- **No onboarding UI** — vault must exist at `Documents/Flynt` before launch
...
6. Vault opens with all cloned content indexed
...
3. Mobile app reads the vault from `Documents/Flynt`
...
|------|----------|---------|
| Launcher profile | `~/.local/share/flynt/launcher-profile.json` | Last vault, known vaults, wizard state |
| Vault config | `{vault}/.flynt/config.toml` | Name, sync, appearance, runtime |
| Omegon profile | `{vault}/.omegon/profile.json` | Model, thinking level, max turns |
| Operator settings | `{vault}/.flynt/operator-settings.json` | Persona, skills, Vox, daemon config |
...
#    Enter: https://github.com/your-org/your-vault.git
#    Branch: main
