...
- Vault config dir: `.flynt/`
- Local state dir: `.flynt-local/`
- Index database: `flynt-index.db`
- Env vars: `FLYNT_VAULT`, `FLYNT_LOCAL_STATE`, `FLYNT_LAUNCHER_PROFILE`, `FLYNT_BUILD_HASH`
...
- `Vault::open()` checks for `.flynt/config.toml` first
...
- **2026-05-05:** Full rename from Codyx to Flynt. Positioning as Obsidian competitor. All internal infrastructure (crates, config paths, env vars) renamed. Auto-migration added for existing vaults. Bundle ID kept as `io.styrene.codex` to preserve TestFlight continuity — will change to `io.styrene.flynt` when a stable build ships.
