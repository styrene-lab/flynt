...
fn process_tag(
    ...
    if tag_lower.starts_with("a ") || tag_lower == "a" {
        // Check for wikilink (data-codex-note attribute)
        if let Some(slug) = extract_attr(&tag_lower, "data-codex-note") {
...
mod tests {
    ...
    #[test]
    fn wikilinks() {
        let html = r##"<p>See <a href="#" data-codex-note="design">design</a>.</p>"##;
        assert_eq!(html_to_markdown(html), "See [[design]].\n");
    }

    #[test]
    fn wikilink_with_display() {
        let html = r##"<p><a href="#" data-codex-note="some%20note">display text</a></p>"##;
