...
//! The migration copies vault content (all files except `.codex-local/`),
//! writes a new config at the destination, and returns the new root path.

use anyhow::{Context, Result};
use codex_core::models::{SyncConfig, VaultConfig};
...
/// Copy all vault files to a new directory, excluding `.codex-local/` and `.git/`.
fn copy_vault(src: &Path, dst: &Path) -> Result<usize> {
    ...
    for entry in walkdir::WalkDir::new(src)
        ...
        .filter_entry(|e| {
            ...
            name != ".codex-local" && name != ".git" && name != ".DS_Store"
        })
    {
        let entry = entry?;
...
mod tests {
    ...
    fn migrate_copies_files_excludes_local_state() {
        ...
        std::fs::create_dir_all(src.join(".codex-local/db")).unwrap();
        std::fs::create_dir_all(src.join(".git/objects")).unwrap();
        std::fs::write(src.join("note.md"), "# Note").unwrap();
        std::fs::write(src.join(".codex/config.toml"), "vault_name = \"test\"").unwrap();
        std::fs::write(src.join(".codex-local/db/index.db"), "binary").unwrap();
        ...
        assert!(dst.join(".codex/config.toml").exists());
        assert!(!dst.join(".codex-local").exists());
