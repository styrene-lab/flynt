...
struct CodexAcpClient {
    tx: EventSender,
}

#[async_trait::async_trait(?Send)]
impl Client for CodexAcpClient {
...
impl AcpSession {
    ...
    pub async fn connect(
        ...
        let mut child = Command::new(&omegon_binary)
            .arg("acp")
            .arg("--cwd")
            .arg(&cwd)
            .arg("-y")
        ...
        let init_resp = conn
            .initialize(
                InitializeRequest::new(agent_client_protocol::ProtocolVersion::LATEST)
                    .client_info(agent_client_protocol::Implementation::new("codex", "0.1.0")),
            )
            .await
    ...
    pub async fn prompt(&self, text: &str) {
        ...
        if let Err(e) = self.conn.prompt(req).await {
            let _ = self.tx.send(AcpEvent::Error(format!("{e}")));
        }
    }
