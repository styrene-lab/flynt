...
//!
//! This module lets the Flynt settings UI manage API keys and trigger OAuth
...
/// Map of hostname patterns to provider IDs.
const HOST_TO_PROVIDER: &[(&str, &str)] = &[
    ("github.com", "github"),
...
pub fn provider_for_url(url: &str) -> Option<&'static str> {
    ...
    HOST_TO_PROVIDER
        .iter()
        .find(|(h, _)| host.eq_ignore_ascii_case(h))
        .map(|(_, id)| *id)
}

/// Look up a stored PAT or OAuth access token for a git remote URL.
/// Returns `None` if no credentials are stored for the resolved provider.
pub fn token_for_url(url: &str) -> Option<String> {
    let provider_id = provider_for_url(url)?;
    ...
    let store = load_auth_store();
    match store.providers.get(provider_id)? {
        StoredCredential::ApiKey { key } if !key.is_empty() => Some(key.clone()),
        StoredCredential::Oauth { access, .. } if !access.is_empty() => Some(access.clone()),
        _ => None,
    }
}

/// Save an OAuth token for a provider (used by device flow).
...
mod tests {
    ...
    #[test]
    fn token_for_url_from_auth_json() {
        // token_for_url reads from auth.json; we can't easily test without
        // overriding the path, but verify it doesn't panic on unknown URLs
        assert_eq!(token_for_url("https://unknown-host.example/repo.git"), None);
    }
