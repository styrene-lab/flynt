...
use flynt_core::{
    graph::{
        GraphEdgeKind, GraphNodeKind, LayoutConfig, build_graph_payload, force_layout,
        render_graph_svg,
    },
    models::*,
...
fn test_search_documents() {
    ...
    assert!(
        !results.is_empty(),
        "Search for 'architecture' returned nothing"
    );
...
fn test_get_document_by_path() {
    ...
    let doc = project
        .store
        .get_document_by_path(Path::new("Welcome.md"))
        .unwrap();
    assert!(doc.is_some());
    let doc = doc.unwrap();
...
fn test_backlinks() {
    ...
    let arch = project
        .store
        .find_document_by_slug("architecture")
        .unwrap()
        .unwrap();
    let backlinks = project.store.get_backlinks(&arch.id).unwrap();
    assert!(
        backlinks.len() >= 2,
        "Expected ≥2 backlinks to Architecture, got {}",
        backlinks.len()
    );
}
...
fn test_graph_payload() {
    ...
    assert!(
        graph
            .nodes
            .iter()
            .any(|n| n.kind == GraphNodeKind::Document)
    );
    ...
    assert!(
        graph
            .edges
            .iter()
            .any(|e| e.kind == GraphEdgeKind::Wikilink)
    );
...
fn test_graph_with_boards_and_tasks() {
    ...
    assert!(
        graph
            .edges
            .iter()
            .any(|e| e.kind == GraphEdgeKind::TaskMembership)
    );
}
...
fn test_orphan_nodes() {
    ...
    let degree = graph
        .edges
        .iter()
        .filter(|e| e.source == orphan.id || e.target == orphan.id)
        .count();
    ...
    let engines = graph
        .nodes
        .iter()
        .find(|n| n.title == "Game Engines")
        .unwrap();
    let eng_degree = graph
        .edges
        .iter()
        .filter(|e| e.source == engines.id || e.target == engines.id)
        .count();
...
fn test_store_memory_fact() {
    let (tmp, project) = setup_project();
    let path = project
        .store_memory_fact("testing", "Test Fact", "This is a test memory fact.")
        .unwrap();
...
fn test_store_communication() {
    let (tmp, project) = setup_project();
    let path = project
        .store_agent_communication("testing", "Test Comm", "Agent communication content.")
        .unwrap();
    assert!(tmp.path().join(&path).exists());
...
fn test_rename_document_updates_links() {
    ...
    let files_updated = project
        .rename_document(Path::new("Projects.md"), "Active Projects")
        .unwrap();
    ...
    assert!(
        welcome.contains("[[Active Projects]]"),
        "Welcome should link to Active Projects, got: {welcome}"
    );
    assert!(
        !welcome.contains("[[Projects]]"),
        "Welcome should not have old link"
    );

    // At least Welcome.md was updated
    assert!(
        files_updated >= 1,
        "Expected at least 1 file updated, got {files_updated}"
    );
}
...
fn test_rename_preserves_display_links() {
    ...
    assert!(
        linker.contains("[[Active Projects|our projects]]"),
        "Display link should be preserved: {linker}"
    );
}
...
fn test_decay_relevance_natural_7day() {
    ...
    assert!(
        r > 0.45 && r < 0.55,
        "After 7 days natural decay should be ~0.5, got {r}"
    );
}
...
fn test_decay_custom_rate() {
    ...
    assert!(
        r > 0.45 && r < 0.55,
        "After 1 day with 1-day half-life should be ~0.5, got {r}"
    );
}
...
fn test_mark_notification_delivered() {
    ...
    let delivered_path = tmp
        .path()
        .join(format!(".flynt/notifications/delivered/{id}.json"));
...
fn test_check_task_notifications_due_date() {
    ...
    assert!(
        notifications
            .iter()
            .any(|n| n.title == "Due today" && n.kind == NotificationKind::DueDate)
    );
}
...
fn test_reindex_skips_flynt_dir() {
    ...
    std::fs::write(
        project.root.join(".flynt/internal.md"),
        "# Should be ignored",
    )
    .unwrap();
    project.reindex().unwrap();
...
fn test_rename_tag_nonexistent() {
    ...
    project.reindex().unwrap();
    let count = project
        .rename_tag("nonexistent-tag-xyz", "new-tag")
        .unwrap();
    assert_eq!(count, 0);
}
...
fn test_merge_tags() {
    ...
    project
        .save_document_content(&p1, "+++\ntitle = \"M1\"\ntags = [\"src1\"]\n+++\n\nBody.")
        .unwrap();
    project
        .save_document_content(&p2, "+++\ntitle = \"M2\"\ntags = [\"src2\"]\n+++\n\nBody.")
        .unwrap();
...
fn test_check_task_notifications_decay() {
    ...
    assert!(
        notifications
            .iter()
            .any(|n| n.kind == NotificationKind::Decay),
        "expected decay notification for fading task (relevance ~0.25), got: {:?}",
        notifications
    );
}
...
fn test_check_task_notifications_skips_done() {
    ...
    let notifications = project.check_task_notifications().unwrap();
    assert!(
        !notifications.iter().any(|n| n.title == "Completed"),
...
fn test_search_documents_finds_match() {
    ...
    assert!(
        !results.is_empty(),
        "search for 'Welcome' should find the welcome doc"
    );
}
...
fn test_get_backlinks() {
    ...
    assert!(
        backlinks.iter().any(|bl| bl.title == "Welcome"),
        "expected Welcome in backlinks of Projects, got: {:?}",
        backlinks.iter().map(|b| &b.title).collect::<Vec<_>>()
    );
}
...
fn test_delete_document_removes_file_and_index() {
    ...
    assert!(
        project.store.get_document(&target.id).unwrap().is_none(),
...
fn test_delete_document_does_not_affect_others() {
    ...
    assert!(
        project.store.get_document(&other.id).unwrap().is_some(),
...
fn test_switch_project_opens_different_content() {
    ...
    let tmp1 = tempfile::Builder::new()
        .prefix("flynt-project1-")
        .tempdir()
        .unwrap();
    let tmp2 = tempfile::Builder::new()
        .prefix("flynt-project2-")
        .tempdir()
        .unwrap();
...
fn test_excalidraw_files_not_indexed_as_documents() {
    ...
    assert_eq!(
        docs.len(),
        1,
        "only .md files should be indexed, got: {:?}",
...
fn test_publication_private_not_exported() {
    ...
    if output.join("manifest.json").exists() {
        ...
        let docs = manifest["documents"].as_array().unwrap();
        assert!(
            !docs.iter().any(|d| d["title"] == "Private"),
...
fn test_publication_policy_rules_tag_match() {
    ...
    config.publication.rules = vec![flynt_core::models::PublicationRule {
    ...
    assert!(
        !titles.contains(&"Untagged"),
        "untagged doc should NOT be exported (default private)"
    );
}
...
fn test_publication_policy_rules_path_prefix() {
    ...
    assert!(
        ...
        "path-matched doc should be exported"
    );
    assert!(
        !titles.contains(&"Hidden"),
        "non-matched doc should NOT be exported"
    );
}
...
fn test_publication_wikilink_to_private_becomes_plain_text() {
    ...
    assert!(
        !exported_md.contains("[[Private Note]]"),
        "wikilink to private doc should be rewritten"
    );
    assert!(
        exported_md.contains("Private Note"),
        "text should remain, just not as a link"
    );
}
